...
class TokenAuthenticationFilter : OncePerRequestFilter() {
	...
	override fun doFilterInternal(
		...
		when (dummyAuthId == "1") {
			true -> {
				...
				val authentication =
					UsernamePasswordAuthenticationToken(dummyUser, dummyUser.password, dummyUser.authorities)
	...
	private fun validTokenProcess(accessToken: String, request: HttpServletRequest) {
		try {
			...
			val authentication =
				UsernamePasswordAuthenticationToken(userAccount, userAccount.password, userAccount.authorities)
			authentication.details = WebAuthenticationDetailsSource().buildDetails(request)
			SecurityContextHolder.getContext().authentication = authentication
